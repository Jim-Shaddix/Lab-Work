...
% LORENTZ_FIT_GUESS: 
% returns the fit parameter guesses for a Lorentzian function 
% for each peak in a file.
%
% PARAMETERS:
% 1. cell_x_peak: where the peak occured on the x-axis
% 2. y_peak_real: where the peak occured on the y-axis (real)
% 3. y_peak_imag: where the peak occured on the y-axis (imag)
    ...
    for i = 1:length(cell_x_peak)
        ...
        cell_params = {128, ...
         ...
         [A, theta, fn, x_offset, y_offset] = cell_params{:};
%         
        %%
        % SET: return value
        guesses{i} = [A, theta, fn, x_offset, y_offset];
