function data = Set_Raw_Peaks(tdms_data)
...
% - this function currently picks to many peaks
    ...
    for i = 1:length(tdms_data)
        ...
        for j = 1:length(xpeak)
            tdms_data(i).set_peaks(j).x_signal      = xpeak(j);
            tdms_data(i).set_peaks(j).x_frequencies = xpeak_freq(j);
        end
        ...
        for j = 1:length(ypeak)
            tdms_data(i).set_peaks(j).y_signal      = ypeak(j);
            tdms_data(i).set_peaks(j).y_frequencies = ypeak_freq(j);
        end
